...
router.post("/create", auth.authenticateToken, upload.single("image"), async (req, res) => {
    ...
    let Article = await queries.createArticle(article.title, article.description, article.price, article.image);
...
router.put("/update/:id", auth.authenticateToken, async (req, res) => {
    ...
    let result = await queries.updateArticle(article.title, article.description, article.price, article.id);
